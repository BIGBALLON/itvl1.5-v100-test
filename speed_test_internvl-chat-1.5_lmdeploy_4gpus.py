...
if __name__ == '__main__':
    ...
    for count, img_path in enumerate(IMAGE_LIST, 1):
        ...
        response = pipe((question, load_image(
            os.path.join(IMAGE_ROOT, img_path))))
        end_loop_time = time.time()
        loop_time = end_loop_time - start_loop_time
        response_len += len(response.text)
        print(f" == [{count}] {img_path}: took {loop_time:.2f} seconds, response length: {len(response.text)}, generate_token_len: {response.generate_token_len}")
        print(f"Question: {question} Response: {response.text}")
